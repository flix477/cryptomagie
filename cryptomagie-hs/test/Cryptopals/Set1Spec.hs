module Cryptopals.Set1Spec (spec) where
...
import qualified Analysis.XOR
  ( findByteKey
  , findByteKey'
  , findRepeatingKey
  )
import qualified Encoding.Base64 as Base64 (encode, decode)
  ...
  describe "Set 1" $ do
    ...
    describe "Challenge 3 - Single-byte XOR cipher" $ do
      it "should return the right XOR key and message" $ do
        ...
        let Just (key, msg, _) = Analysis.XOR.findByteKey $ Hex.decode input
    ...
    describe "Challenge 4 - Detect single-character XOR" $ do
      it "should find the correct single byte XOR'd line" $ do
        f <- readFile "./test/Cryptopals/challenge4.txt"
        let Just (key, msg, _) = Analysis.XOR.findByteKey' $ map Hex.decode $ lines f
        let expectedKey = 53
    ...
    describe "Challenge 6 - Break repeating-key XOR" $ do
      it "should decrypt the input file correctly" $ do
        f <- readFile "./test/Cryptopals/challenge6.txt"
        let Just input = Base64.decode $ concat $ lines f
        let (output:_) = Analysis.XOR.findRepeatingKey input
        BT.putStrLn output
